    ...
    # Random analysis for low frequencies
        ...
        for h, file in enumerate(files):        # Random interval
            ...
            for i in range(len(interval_random)):  # Iterate through the random range
                ...
                if rand == 0:
                    bbe = calc_bbe(file, log, options.QS, options.QH, options.S_freq_cutoff, options.H_freq_cutoff, options.temperature, 0, 0, num_files, rand, 
                                   conc, options.freq_scale_factor, options.freespace, options.spc, options.invert,
                                   0.0, cosmo=cosmo_option, inertia=options.inertia, g4=options.g4)
                    interval_bbe_data[h].append(bbe)
                else:
                    bbe = calc_bbe(file, log, options.QS, options.QH, options.S_freq_cutoff, options.H_freq_cutoff, options.temperature, value_up, freq_range, num_files, rand, 
                                   conc, options.freq_scale_factor, options.freespace, options.spc, options.invert,
                                   0.0, cosmo=cosmo_option, inertia=options.inertia, g4=options.g4)
                    interval_bbe_data[h].append(bbe)
